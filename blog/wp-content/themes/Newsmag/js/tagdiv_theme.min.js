...
new m.args("complete",b,a(c),h,d,d);a(c).parent().data("args",h);if(""!=b.onSlideComplete)b.onSlideComplete(h)},getSliderOffset:function(b,c){var h;c="x"==c?4:5;if(!f||p||w)h=parseInt(a(b).css("left"),10);else{h=["-webkit-transform","-moz-transform","transform"];for(var d,e=0;e<h.length;e++)if(void 0!=a(b).css(h[e])&&0<a(b).css(h[e]).length){d=a(b).css(h[e]).split(",");break}h=void 0==d[c]?0:parseInt(d[c],10)}return h},setSliderOffset:function(b,c){c=parseInt(c,10);!f||p||w?a(b).css({left:c+"px"}):
...
2;c+g.outerHeight(!0)>window.innerHeight&&(c-=c+g.outerHeight(!0)-window.innerHeight);l=l.length?l.outerHeight(!0):0;l>c&&(c=l);l={top:c,right:f,"padding-right":p,width:""};c=g.data("width-preview");0<p&&(l.width=c+p);g.css(l);g.data("right-value",f+p);g.css("visibility","visible")},function(c){a(".td-screen-demo-extend:first").hide();var d=a(".td-screen-demo:first");c=d.css("right");var e=d.css("padding-right"),g=d.css("width"),l=parseInt(c.replace("px","")),f=parseInt(e.replace("px","")),p=parseInt(g.replace("px",
...
parseInt(d.outerWidth())&&(f=!0));else{var p=d.prev(".td-set-theme-style");p.length&&parseInt(p.position().top)+parseInt(a(window).scrollTop())<tdDemoMenu.mousePosY&&tdDemoMenu.mousePosY<parseInt(p.position().top)+parseInt(a(window).scrollTop())+parseInt(p.outerHeight())&&(l=!0,parseInt(a(window).width())-parseInt(d.outerWidth())<tdDemoMenu.mousePosX&&tdDemoMenu.mousePosX<parseInt(a(window).width())&&(f=!0))}if(l&&f)return c=e,!1});b===c?a("#td-theme-settings").find(".td-screen-demo:first").css("visibility",
